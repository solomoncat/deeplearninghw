...
class LeakyReLU(Layer):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        dx = dout * torch.where(x >= 0.0, 1.0, self.alpha)
        # ========================
...
class Sigmoid(Layer):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        x = self.grad_cache["x"]
        exp_f = torch.exp(-x)/(1+torch.exp(-x))**2
        dx = dout*exp_f
        # ========================
...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        # ====== YOUR CODE: ======
        loss = torch.sum(-x[range(N),y] + torch.log(torch.sum(torch.exp(x), axis = 1)))
        loss /= N
        # ========================
    ...
    def backward(self, dout=1.0):
        ...
        # ====== YOUR CODE: ======
        exp_d = torch.exp(x)/(torch.sum(torch.exp(x), keepdim=True, axis=1))
        exp_d[range(N),y] -= 1
        dx = dout*exp_d/N
        # ========================
...
class MLP(nn.Module):
    def __init__(
        ...
        **kw,
        ...
        activations = hw2.mlp.ACTIVATIONS
        activation_cls = activations[activation]
        activation = activation_cls(**ACTIVATION_DEFAULT_KWARGS.get(activation, {}))
        ...
        # Output layer
        layers.append(nn.Linear(hidden_features[-1], num_classes))

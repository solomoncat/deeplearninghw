...
class ClassifierTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        # Classify and calculate the number of correct predictions
        with torch.no_grad():
        ...
        return BatchResult(loss=batch_loss, num_correct=num_correct)
...
class LayerTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        # ====== YOUR CODE: ======
        #forwads:
        x_score = self.model(X.reshape(X.shape[0],-1))
        loss = self.loss(x_score, y)

        #backwards:
        self.opt.zero_grad()
        back_loss = self.loss.backward(loss)
        self.model.backward(back_loss)
        #optimize:
        self.opt.step()

        #results:
        new_score = self.model(X.reshape(X.shape[0],-1))
        y_pred = torch.argmax(new_score, dim=1)
    ...
    def test_batch(self, batch) -> BatchResult:
        ...
        # ====== YOUR CODE: ======
        x_score = self.model(X.reshape(X.shape[0],-1))
        y_pred = torch.argmax(x_score, dim=1)
        num_correct = torch.sum(y == y_pred).item()
        loss = self.loss(x_score, y)
        # ========================
